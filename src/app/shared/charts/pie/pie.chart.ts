...
import * as Configuration from "./pie.chart.config";
...
export class PieChart extends BaseChart {
	...
	draw() {
		...
		this.path = this.innerWrap.selectAll("path")
			...
			.attr("stroke", Configuration.pie.default.strokeColor)
	...
	generateTooltipHTML(label, val: number) {
		return "<ul class='list-unstyled mb-1'>" +
			"<li>" + "<div class='font-size-base my-1'>" + label + "</div>" + "</li>" +
			"<li>" + "Total: &nbsp;" + "<span class='font-weight-semibold float-right'>" + val + "</span>" + "</li>" +
			"<li>" + "Share: &nbsp;" + "<span class='font-weight-semibold float-right'>" + (100 / (this.datasetStats().sum / val)).toFixed(2) + "%" + "</span>" + "</li>" +
			"</ul>" +
	...
	private computeDiameterAndRadius() {
		let diameter: number;
		let radius : number;
		//if (Configuration.pie.radius) {
			const chartSize: any = this.getChartSize(this.container);
			diameter= Math.min(chartSize.width - 30, chartSize.height -30);
			radius = diameter / 2.5
		//}
		//else {
		//	diameter= this.options.config.chartConfig.pie.radius * 2;
		//	radius = this.options.config.chartConfig.pie.radius;
		//}
