...
// CaseManager Dashboard Component
export class CaseManagerDashboardComponent implements OnInit {
  ...
  constructor(private toastr: ToastrService, private svcCaseManager: CaseManagerService, private urlConstantService: UrlconstantsService, private fb: FormBuilder, public messageService: SharedService) {
            ...
            self.pieChartConfig = {
              ...
              "data": self.memberStatsByState[code].map(data => {
                ...
                switch (data.status) {
                  ...
                  case "No Eligible": _color = "#29B6F6"; break;
                }
                return {
  ...
  getMembershipStats() {
    ...
    this.svcCaseManager.getMembershipStats(url).subscribe(memberStats => {
      if (memberStats) {
        ...
        this.referred = this.eligible + this.pending + this.noteligible;
      }
    }, (error) => {
  ...
  getMembershipStatsByState() {
    ...
    this.svcCaseManager.getMembershipStatsByState(url).subscribe(memberStats => {
      if (memberStats) {
        this.membersByStatusList = memberStats;
